...
function onMouseMove(event) {
  ...
  allHoverableObjects.forEach((hoverableObject) => {
    hoverableObject.material.forEach((element) => { element.opacity = 1; })
  })

	const intersection = calcIntersection(event);
  if (typeof intersection !== 'undefined' && typeof intersection.object.userData.length !== 'undefined' && !isMouseDown) {
    // reduce the opacity for all the intersection object materials
    intersection.object.material.forEach((element) => { element.opacity = 0.9; })
...
function onMouseDown( event ) {
  ...
  if (typeof intersection !== 'undefined' && typeof intersection.object.userData.length !== 'undefined') {
...
function onMouseUp( event ) {
  ...
  if (typeof intersection !== 'undefined' && typeof intersection.object.userData.length !== 'undefined' &&  timeSinceMouseDown < 200) {
...
function creatPageMaterials(frontImage, sfBack = true) {
  ...
  if (sfBack) {  // if the page materials should include a back with the Salesforce logo
  ...
  return [backMaterial, frontMaterial, sidesMaterial, sidesMaterial, sidesMaterial, sidesMaterial];
}
...
const box1 = new THREE.Mesh(boxGeo, creatPageMaterials('data/automations/validation_rules.png'));
box1.position.set(-19, 0, -145);
box1.rotation.set(0, 50 * degToRad, 0);
box1.userData = 'https://help.salesforce.com/s/articleView?id=sf.fields_about_field_validation'
scene.add(box1);

const box2 = new THREE.Mesh(boxGeo, creatPageMaterials('data/automations/approvals.png'));
box2.position.set(32, 0, -80);
box2.rotation.set(0, 25 * degToRad, 0);
box2.userData = 'https://help.salesforce.com/s/articleView?id=sf.what_are_approvals'
scene.add(box2);

const box3 = new THREE.Mesh(boxGeo, creatPageMaterials('data/automations/workflows.png'));
box3.position.set(50, 0, 0);
box3.userData = 'https://help.salesforce.com/s/articleView?id=sf.customize_wf'
scene.add(box3);

const box4 = new THREE.Mesh(boxGeo, creatPageMaterials('data/automations/processes.png'));
box4.position.set(32, 0, 80);
box4.rotation.set(0, -25 * degToRad, 0);
box4.userData = 'https://help.salesforce.com/s/articleView?id=sf.process_overview'
scene.add(box4);

const box5 = new THREE.Mesh(boxGeo, creatPageMaterials('data/automations/flows.png'));
...
const creditBox = new THREE.Mesh(creditBoxGeo, creatPageMaterials('data/credit.png', false));
...
function animate() {
  ...
  stars.forEach(element => (element instanceof THREE.Mesh) ? element.position.applyAxisAngle(rotationAxis, 0.002 * degToRad) : element);
